...
export default function IntegrationsPage() {
  ...
  const saveOperatorConfig = async (operatorCode, values) => {
    ...
    try {
      if (operatorCode === 'USSD_SIMULATOR') {
        ...
        const config = {
          apiUrl: values.apiUrl || 'http://104.207.142.188:3001',
  ...
  const BankConfigForm = ({ bank }) => {
    ...
    const initialValues = savedConfig ? JSON.parse(savedConfig) : 
      bank.code === 'GHW' ? {
        apiUrl: 'http://104.207.142.188:4500',
  ...
  const OperatorConfigForm = ({ operator }) => {
    ...
    const initialValues = savedConfig ? JSON.parse(savedConfig) : 
      operator.code === 'USSD_SIMULATOR' ? {
        apiUrl: 'http://104.207.142.188:3001',
        timeout: 30000,
        enabled: true,
      } : {};
        ...
        >
          {operator.code === 'EMOLA' && (
            <>
              ...
              <Form.Item label="URL Simulador USSD" name="ussdSimulatorUrl" initialValue="http://104.207.142.188:3001">
                <Input prefix={<ApiOutlined />} placeholder="http://104.207.142.188:3001" />
              </Form.Item>

// Importa clientes de um arquivo JSON (exportado do localStorage) para o banco Prisma do simulador
const { PrismaClient } = require('@prisma/client');
const fs = require('fs');

const prisma = new PrismaClient();
const JSON_PATH = 'simulator-customers.json';

async function importCustomers() {
    try {
        if (!fs.existsSync(JSON_PATH)) throw new Error('Arquivo JSON não encontrado: ' + JSON_PATH);
